...
- 先頭 Body の質量は 48.00 以上 120.00 以下で生成する。
- 2 体目以降の Body の質量は 0.05 以上 6.00 以下で生成する。
- 先頭 Body の質量は必ず 2 体目以降の全 Body より大きくなること。
...
- `random-cluster` で Generate した場合、先頭 Body の質量が 48.00 以上 120.00 以下で、2 体目以降の Body の質量が 0.05 以上 6.00 以下であること
...
- Simulation metrics panel の値が 1 秒以内の遅延で更新されること
...
### 7.1.4 Visualization metrics panel 要件

- Simulation metrics panel は viewport-stage の直下に sibling として配置し、Canvas に重ねない。
- Large では metrics panel が viewport-stage と同じ列幅を使い、canvas の可視面積を削らないことを優先する。
- metrics panel の内側余白は 8px 前後、主要値フォントは 1rem 以下を目安とする。
- Reproducibility Key を含む長い値は省略または折り返しを許容するが、panel 外へはみ出さないこと。
...
- 1440px x 1024px の表示で Control panel 高さが 220px 以下、Simulation metrics panel が viewport-stage と重ならず、その高さが 220px 以下であること
