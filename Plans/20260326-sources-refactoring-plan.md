...
- `PERSISTENCE_POLICY` により、保存対象と保存しない対象をコードと文書の両方で固定する
...
Phase 2 の persistence 境界では、`PERSISTENCE_POLICY` として少なくとも以下の保存対象を固定する。
...
Phase 2 の persistence 境界では、`PERSISTENCE_POLICY` として少なくとも以下の保存しない対象を固定する。
...
bootstrap fail-fast 時は、部分初期化済み resource を cleanup して副作用を残さない。
