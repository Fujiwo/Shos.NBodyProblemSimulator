...
- 測定条件:
  ...
  - 追加比較条件は Chromium、`showTrails=true`、1280x900、各シナリオ 30 秒で固定した
...
### Expanded Condition Check

追加の比較は、同一の Playwright 手順で `Generate -> Start -> 30 秒待機 -> Pause` を行って取得した。

結果要約:

- `random-cluster`, `velocity-verlet`, `bodyCount=10`
  - main: FPS 17.42, simulationTime 8.94, pipelineTime 0.00 ms
  - worker: FPS 11.61, simulationTime 7.25, pipelineTime 15.00 ms
- `random-cluster`, `rk4`, `bodyCount=10`
  - main: FPS 17.42, simulationTime 8.63, pipelineTime 0.20 ms
  - worker: FPS 16.88, simulationTime 8.18, pipelineTime 12.40 ms
- `sample`, `velocity-verlet`, `bodyCount=8`
  - main: FPS 13.55, simulationTime 8.42, pipelineTime 0.00 ms
  - worker: FPS 15.00, simulationTime 8.21, pipelineTime 12.60 ms
- `sample`, `rk4`, `bodyCount=8`
  - main: FPS 17.42, simulationTime 8.31, pipelineTime 0.10 ms
  - worker: FPS 15.49, simulationTime 7.46, pipelineTime 28.20 ms

追加観察:

- `sample` と `random-cluster` の差があっても、main の pipeline time は常に 0.00 から 0.20 ms に収まっている
- `rk4` に切り替えても、main-thread 条件では physics 計算が支配的コストになった証拠は弱い
- worker は全条件で main より pipeline time が大きく、特に `sample + rk4` では 28.20 ms まで悪化した
- したがって、条件を広げても「まず main-thread の UI/render 更新を減らす」という優先順位は変わらない
...
- 追加測定でも worker pipeline time は 12.40 ms から 28.20 ms で、`sample + rk4` ではむしろ悪化幅が大きい
...
- RK4 条件も追加確認したが、main pipeline time は `random-cluster + rk4` で 0.20 ms、`sample + rk4` で 0.10 ms に留まり、優先度を上げる根拠は依然として弱い
...
### random-cluster + velocity-verlet

1. UI full rerender と store clone
2. trail 更新コスト
3. worker payload overhead
4. physics allocation

理由:

- main pipeline time はほぼ 0 ms で、physics より main-thread 更新の無駄を先に疑うべき条件である
- worker は main より悪く、通信改善は必要だが第 2 段でよい
...
- RK4 でも main pipeline time は 0.20 ms と小さい
- physics の計算量は増えているが、まだ優先順位を逆転させるほどではない

### sample + velocity-verlet
...
- bodyCount が 8 に下がっても main pipeline time は 0 ms で、傾向は変わらない
- sample の fixed dataset でも UI と render の固定負荷が目立つ

### sample + rk4

1. UI full rerender と store clone
2. worker payload overhead
3. trail 更新コスト
4. physics allocation

理由:

- この条件では worker pipeline time が 28.20 ms まで上がり、worker 通信設計の問題が trail より前に出る
- ただし main pipeline time は 0.10 ms なので、実装の第 1 手としては依然 UI/render 側の整理を優先するのが安全である
...
- `npm run test:ui`
- `npm run benchmark:phase4`
- custom Playwright benchmark for `sample` / `rk4` / `random-cluster` scenario expansion