...
- 現行 baseline の採用積分法は Velocity Verlet とすること
...
- `sun.jpg`、`mercury.jpg`、`venus.jpg`、`earth.jpg`、`moon.jpg`、`mars.jpg`、`jupiter.jpg`、`saturn.jpg` を現行 baseline の利用対象として扱えること
...
- compact controls の visible text、validation panel の表示条件、per-body Open / Closed toggle を持つ body card の UI 方針を壊さないこと
...
2. deterministic validation
	- 同一 Generate 結果から Start した時、最初の数十 step の挙動が同一 build で再現可能であること
