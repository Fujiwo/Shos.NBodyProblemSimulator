...
今回の目的は、現行の Phase 3 実装を前提に、積分法比較と性能改善判断に必要な基盤を追加することです。

この prompt における Phase 4 は、古い計画書に残っている「3D 描画」ではなく、現行コードベースで未実装の残タスクを指します。具体的には以下を実装対象とします。
...
Worker 有効化の最終製品判断は性能測定に基づいて行ってください。初回実装では、少なくとも以下のどちらかを必須とします。
