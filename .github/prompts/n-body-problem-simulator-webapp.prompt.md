...
このリポジトリに対して、ブラウザ上で動作する「3D N-body problem simulator(N体問題シミュレータ)」を段階的に実装するための【開発計画書】を作成し、あわせて GitHub Copilot 用のリポジトリ内カスタマイズファイルを整備してください。
...
# 段階的なファイル構成案(今回は計画書内に構成案として記載するのみ)
- `Sources/index.html`
- `Sources/style.css`
- `Sources/main.js`
...
5. 描画方式の比較と採用方針(WebGL/3Dライブラリの選定理由)
   - `Sources/images/` 配下の画像ファイルを texture material に使う方針と fallback 方針を明記すること
